...
import { Spin, Form, Button, Row, Col, Input, DatePicker, TimePicker, Select, TreeSelect, InputNumber, Cascader } from 'antd';
...
const getDropDownFrom = (fld, required, edite, props) => { // 5：下拉
  return {
    ...
    component: (
      <Select showSearch placeholder={edite && `请选择${fld.alias}` || '<空>'} dropdownStyle={{ maxHeight: 400, overflow: 'auto' }} filterOption={filterOption} disabled={!edite} {...props} >{(fld.values || []).map((obj, i) => <Option key={`${i * 1}`} value={obj.dbval}>{obj.dispval}</Option>)}</Select>
    ),
  };
};
  ...
  return {
    ...
    component: (
      <TreeSelect showSearch treeDefaultExpandAll placeholder={edite && `请选择${fld.alias}` || '<空>'} dropdownStyle={{ maxHeight: 400, overflow: 'auto' }} filterTreeNode={filterTreeNode} disabled={!edite} {...props}>
        {(fld.values || []).map(node => getNode(node, fld.kv || {title: 'title', key: 'key', value: 'value'}))}
      </TreeSelect>
    ),
  };
};
  ...
  render() {
    ...
    const items = flds.map((x, i) => {
      ...
      return {
        local,
        fld: x.name,
        value: getValue(x.disptype * 1, data[x.name] || data[x.name.toLowerCase()]),
    ...
    const rows = orderBy(Object.keys(group).reduce((a, b) => { return a.concat({idx: b * 1, data: group[b]}); }, []), 'idx').map(o => o.data);
    return (
