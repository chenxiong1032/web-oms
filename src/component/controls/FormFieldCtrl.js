...
const matchExtend = [ // 支持的操作符
  { key: '~', name: '正则', getValue: (key, value) => [key, '~', `'${value}'`]},
  { key: 'in', name: '在列表', getValue: (key, value) => [key, 'in', `(${value.map(x => (isNaN(x) ? `'${x}'` : x)).join(',')})`]},
...
const fieldType = { // 支持的类型
  ...
  dropdown: {
    match: ['=', 'like', 'in', '~', 'emptyAndNull', 'unemptyAndNull'],
    getForm(fld, value, required, edite, props) {
...
function Mvvm () {
  ...
  const onValueRender = ({field, item, edite}) => {
    ...
    if (type === 'dropdown' && match === 'like' || match === '~') return getFormItem(field, item.key, undefined, {type: 'string', alias: '字段值'}, edite);
    if (type === 'dropdown' && match === 'in') return getFormItem(field, item.key, undefined, item.getValue(item.field), edite, {mode: 'multiple'});
  ...
  const onChangeMatch = ({key, field, value, type}) => {
    ...
    this.setState(items, () => {
      if (type === 'dropdown' && (value === '=' || value === 'in')) setFieldsValue({[`value_${key}`]: undefined});
    });
  };
