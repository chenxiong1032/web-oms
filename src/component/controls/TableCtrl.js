...
function getPagination(page) {
  const {current = 1, pageSize = 50, total = 0, onChange = empty, setSize = empty, sizeList = ['50', '100', '200']} = page;
  return (
    <Pagination
      showSizeChanger
      showQuickJumper
...
export default class Control extends React.Component {
  ...
  componentDidMount() {
    ...
    const resize = () => {
      if (this.needResize()) this.setState({ y: $target.offsetHeight - $target.querySelector('.ant-table-thead').offsetHeight});
